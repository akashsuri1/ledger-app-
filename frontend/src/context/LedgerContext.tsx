...
export function LedgerProvider({ children }: { children: ReactNode }) {
  ...
  const [companySelectionRequired, setCompanySelectionRequired] = useState(
    () => state.workspace.companies.length > 1,
  );
  ...
  function createCompany(data: NewCompany) {
    ...
    setCompanySelectionRequired(false);
    return company;
  }
  ...
  function deleteCompany(companyId: number) {
    commitOrThrow(deleteEmptyCompany(getCurrent(), companyId));
  }

  function canDeleteCompany(companyId: number) {
    const current = getCurrent();
    return (
      current.companies.some((company) => company.id === companyId) &&
      isCompanyLedgerEmpty(current, companyId)
    );
  }
