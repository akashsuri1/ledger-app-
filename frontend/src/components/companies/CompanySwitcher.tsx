...
export default function CompanySwitcher() {
  ...
  const deletableCompanyIds = new Set(
    companies
      .filter((company) => canDeleteCompany(company.id))
  ...
  function handleDeleteCompany() {
    ...
    try {
      ...
      toast.success(`${deletedName} deleted`, {
        description:
          companies.length === 1
            ? "Create a company to continue using LedgerFlow."
            : deletingActiveCompany
              ? "LedgerFlow switched to another company."
              : "The empty company was removed.",
      });
    } catch (error) {
      toast.error(
  ...
  return (
    <>
      ...
      <ManageCompaniesModal
        ...
        deletableCompanyIds={deletableCompanyIds}
      ...
      <ConfirmDialog
        ...
        description={
          companyToDelete
            ? companies.length === 1
              ? `Delete ${companyToDelete.name}? You will return to first-company setup.`
              : `Delete ${companyToDelete.name}? This empty company and its settings will be removed permanently.`
            : "Delete this empty company?"
        }
